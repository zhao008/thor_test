...
"""
No-WriteBack variant of NormalSwizzleM kernel.
...
def swizzle_2d_from_bid_m(M, N, tm, tn, GROUP_SIZE_M, bid):
    ...
    bid_m = first_bid_m + ((bid - group_id * num_bid_in_group) % group_size_m)
...
def swizzle_2d_m(M, N, tm, tn, GROUP_SIZE_M):
    bid = ct.bid(0)
    return swizzle_2d_from_bid_m(M, N, tm, tn, GROUP_SIZE_M, bid)


@ct.kernel(num_ctas=ct.ByTarget(sm_100=2))
def matmul_kernel_m_no_writeback(A, B, checksum,
                                  tm: ConstInt,
                                  tn: ConstInt,
                                  tk: ConstInt):
    """
    No-writeback matmul kernel (M-swizzle variant).
    ...
    M = A.shape[0]
    N = B.shape[1]
    bidx, bidy = swizzle_2d_m(M, N, tm, tn, GROUP_SIZE_M)
    ...
    # ct.sum(accumulator, axis=None) reduces the entire tm×tn tile to one scalar.
    # This ensures the compiler cannot eliminate the MMA loop above.
    tile_checksum = ct.sum(accumulator, axis=None)

    # Compute linear block id for indexing into the checksum buffer
    num_bid_n = ct.cdiv(N, tn)
    block_linear_id = bidx * num_bid_n + bidy

    # Pack scalar into a (1,) tile and store to checksum buffer
