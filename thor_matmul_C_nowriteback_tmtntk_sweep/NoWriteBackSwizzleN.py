...
def swizzle_2d_from_bid_n(M, N, tm, tn, group_size_n, bid):
    ...
    active_group_size_n = min(num_bid_n - first_bid_n, group_size_n)
    bid_n = first_bid_n + ((bid - group_id * num_bid_in_group) % active_group_size_n)
    bid_m = (bid % num_bid_in_group) // active_group_size_n
    return bid_m, bid_n


def swizzle_2d_n(M, N, tm, tn, group_size_n):
    return swizzle_2d_from_bid_n(M, N, tm, tn, group_size_n, ct.bid(0))


@ct.kernel(num_ctas=ct.ByTarget(sm_100=2))
def matmul_kernel_n_no_writeback(
    A,
    B,
    checksum,
    tm: ConstInt,
    ...
    M = A.shape[0]
    N = B.shape[1]
    bid_m, bid_n = swizzle_2d_n(M, N, tm, tn, group_size_n)
